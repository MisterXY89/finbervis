...
def get_max_sal_indicies(merged_saliency, n=3):
    index_max_scores = []
    for i in range(0, n):
        max_el = max(merged_saliency)
        i_max_el = merged_saliency.index(max_el)
        # index_max_scores.append((i_max_el-1, i_max_el, i_max_el+1))
        index_max_scores.append(i_max_el)
...
def inspect_pos_tags(i, log = False):
    ...
    for mi in max_sal_indicies:
        pos_tuple = (pos_tags[mi[0]], pos_tags[mi[1]], pos_tags[mi[2]])
...
# print(b2a[0])
# print(spacy_tokens[0] == bert_tokens[b2a[0][0]])
...
# print("% ID %")
# for s in sents:
#     corr_pred_perc = len(df.query(f"truth_label == '{s}' and sentiment == '{s}'"))/len(df.query(f"truth_label == '{s}'"))
#     print(f"% {s} {corr_pred_perc}")
# print("------")

def label_check(df_):
    for s in sents:
        print(f"% diff {s}: ", len(df_.query(f"sentiment == '{s}'"))/len(df_))            
    print("---")
