...
class Interface:
	...
	def __init__(self):
		...
		self.dfs_ = self.read_dfs()
		self.dists = self.read_dists()
		self.sal_calc = SaliencyCalculator(self.tokenizer, self.sent_pred.model)
		self.load_reducer()
	
	def read_dists(self):
		dists_path = f"{DATA_DIR}/dists.pk"
		if os.path.isfile(dists_path):
			with open(dists_path, "rb") as file:
				return pickle.load(file)
			
		dists = [Dist(df) for df in self.dfs_]	
	...
	def get_text_by_id(self, id, model):
		return list(self.search(seg_id=id, model=model)["segment"])[0]

	def get_similar_sents(self, model, id=0, n=5, return_sents=False):
		print(id)
	...
	def search(self, seg_id=None, q=None, model=0):
		...
		if q[-1] == " ":
			q = q[0:-1]
		return df[df['segment'].str.contains(q)]
